...
import type { CapacityAction } from '../domain/types';
import {
  actionStartDate,
  analyze,
  demand,
...
const growth = () => structuredClone(INITIAL_PLANS.find((p) => p.id === 'growth')!.config);
...
describe('planning demand', () => {
  it('reproduces the specified hard, expected, and scenario demand', () => {
    ...
    expect(rounded(result.hard)).toEqual([18,19,20,20,21,22,22,21,20,19,20,21,21,20,19,18,18,17]);
    expect(rounded(result.expected)).toEqual([19,20,21,22,23,25,25,24,23,22,24,25,24,23,22,21,20,19]);
    expect(rounded(result.scenario)).toEqual([19,20,21,22,23,25,25,26,27,28,32,35,34,31,27,23,20,19]);
  });

  it('supports zero and 100% probability overrides without changing hard backlog', () => {
    const base = demand(growth(), 'Plumber');
    const zero = growth(); zero.probabilities.s1 = 0;
    const full = growth(); full.probabilities.s1 = 100;
    expect(demand(zero, 'Plumber').expected[6]).toBeLessThan(base.expected[6]);
    expect(demand(full, 'Plumber').expected[6]).toBeGreaterThan(base.expected[6]);
    expect(demand(zero, 'Plumber').hard).toEqual(base.hard);
  });

  it('shifts schedules without reshaping values', () => {
    const base = growth(); const shifted = growth(); shifted.shifts.p2 = 2;
    const p2 = PROJECTS.find((p) => p.id === 'p2')!;
    const baseDemand=demand(base,'Plumber').hard,shiftedDemand=demand(shifted,'Plumber').hard;
    expect(shiftedDemand.map((v,i)=>v-baseDemand[i])).toEqual(p2.curve.map((v,i)=>(i>=2?p2.curve[i-2]:0)-v));
...
describe('capacity and bottlenecks', () => {
  ...
  it('supports optional pre-productive ramp capacity', () => {
    ...
    expect(hireRampFactors(action)).toEqual({4: .5, 5: .75});
  });

  it('clips actions beginning outside the planning window', () => {
    const cfg = growth();
    cfg.actions.push({id:'outside',kind:'hire',category:'Plumber',quantity:99,fromIndex:20,toIndex:20,status:'Proposed',confirmed:false,notes:''});
    expect(Math.max(...analyze(cfg,'Plumber').plannedHires)).toBe(4);
  });

  it('handles no available capacity', () => {
    const cfg = growth(); cfg.capacity.Plumber.headcount = 0; cfg.actions = [];
...
describe('dates, units, and work packages', () => {
  ...
  it('calculates hiring and subcontract lead-time dates backward', () => {
    const cfg=growth();
    expect(actionStartDate(cfg.actions.find(a=>a.id==='a1')!).toISOString().slice(0,10)).toBe('2026-10-24');
    expect(actionStartDate(cfg.actions.find(a=>a.id==='a3')!).toISOString().slice(0,10)).toBe('2026-08-20');
  });
  ...
  it('rolls child packages into the parent exactly once', () => {
    ...
    expect(rollupProjectCurve(parent).reduce((a,b)=>a+b,0)).toBe(parent.curve.reduce((a,b)=>a+b,0));
  });

  it('validates package value reconciliation and hybrid execution', () => {
    expect(validateWorkPackages(ATLAS.workPackages)).toEqual([]);
    const bad=structuredClone(ATLAS.workPackages);bad[0].valuePercent=25;bad[1].selfPerformPercent=50;
